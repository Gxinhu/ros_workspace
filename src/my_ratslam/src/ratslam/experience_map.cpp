...
namespace ratslam {
  ...
  bool ExperienceMap::on_create_link(int exp_id_from, int exp_id_to, double rel_rad) {
    ...
    for (unsigned int i = 0; i < experiences[exp_id_from].links_from.size(); i++) {
      if (links[experiences[current_exp_id].links_from[i]].exp_to_id == exp_id_to) return false;
    }

    for (unsigned int i = 0; i < experiences[exp_id_to].links_from.size(); i++) {
      if (links[experiences[exp_id_to].links_from[i]].exp_to_id == exp_id_from) return false;
    }
    ...
    new_link->heading_rad
        = get_signed_delta_rad(current_exp->th_rad, atan2(accum_delta_y, accum_delta_x));
    new_link->facing_rad
        = get_signed_delta_rad(current_exp->th_rad, clip_rad_180(accum_delta_facing + rel_rad));
    ...
    return (
        waypoint_exp_id == -1
            ...
            : sqrt(
                ...
                + (double)pow((experiences[waypoint_exp_id].y_m - experiences[current_exp_id].y_m),
                              2)));
  }
  ...
  double ExperienceMap::get_subgoal_rad() const {
    if (waypoint_exp_id == -1)
      ...
      double curr_goal_rad
          = atan2((double)(experiences[waypoint_exp_id].y_m - experiences[current_exp_id].y_m),
