...
namespace ratslam {
  ...
  ExperienceMap::ExperienceMap(ptree settings) {
    ...
    get_setting_from_ptree(EXP_INITIAL_EM_DEG, settings, "exp_initial_em_deg",
                           90.0);
  ...
  bool ExperienceMap::on_create_link(int exp_id_from, int exp_id_to,
    ...
    for (unsigned int i = 0; i < experiences[exp_id_from].links_from.size();
         i++) {
      if (links[experiences[current_exp_id].links_from[i]].exp_to_id
          == exp_id_to)
        return false;
    }

    for (unsigned int i = 0; i < experiences[exp_id_to].links_from.size();
         i++) {
    ...
    new_link->d
        = sqrt(accum_delta_x * accum_delta_x + accum_delta_y * accum_delta_y);
    new_link->heading_rad = get_signed_delta_rad(
        current_exp->th_rad, atan2(accum_delta_y, accum_delta_x));
    new_link->facing_rad = get_signed_delta_rad(
        current_exp->th_rad, clip_rad_180(accum_delta_facing + rel_rad));
  ...
  int ExperienceMap::on_set_experience(int new_exp_id, double rel_rad) {
    if (new_exp_id > experiences.size() - 1) return 0;
    ...
    accum_delta_y = 0;
    accum_delta_facing
        = clip_rad_180(experiences[current_exp_id].th_rad + rel_rad);
  ...
  double exp_euclidean_m(Experience *exp1, Experience *exp2) {
    return sqrt((double)((exp1->x_m - exp2->x_m) * (exp1->x_m - exp2->x_m)
                         + (exp1->y_m - exp2->y_m) * (exp1->y_m - exp2->y_m)));
  }

  double ExperienceMap::dijkstra_distance_between_experiences(int id1,
                                                              int id2) {
    double link_time_s;
    unsigned int id;

    std::priority_queue<Experience *, std::vector<Experience *>, compare>
        exp_heap;
    ...
    while (!exp_heap.empty()) {
      std::make_heap(
          const_cast<Experience **>(&exp_heap.top()),
          const_cast<Experience **>(&exp_heap.top()) + exp_heap.size(),
          compare());
  ...
  bool ExperienceMap::calculate_path_to_goal(double time_s) {
    ...
    if (exp_euclidean_m(&experiences[current_exp_id],
                        &experiences[goal_list[0]])
            < 0.1
    ...
    if (goal_timeout_s == 0) {
      ...
      while (!exp_heap.empty()) {
        ...
        for (id = 0; id < exp->links_to.size(); id++) {
          ...
          link_time_s = exp->time_from_current_s + link->delta_time_s;
          if (link_time_s
              < experiences[link->exp_from_id].time_from_current_s) {
            experiences[link->exp_from_id].time_from_current_s = link_time_s;
            experiences[link->exp_from_id].goal_to_current = exp->id;
          }
        }
      ...
      while (trace_exp_id != current_exp_id) {
        experiences[experiences[trace_exp_id].goal_to_current].current_to_goal
            = trace_exp_id;
  ...
  void ExperienceMap::add_goal(double x_m, double y_m) {
    ...
    if (MAX_GOALS != 0 && goal_list.size() >= MAX_GOALS) return;
    ...
    if (min_dist < 0.1) add_goal(min_id);
  }

  double ExperienceMap::get_subgoal_m() const {
    return (waypoint_exp_id == -1
                ? 0
                : sqrt((double)pow((experiences[waypoint_exp_id].x_m
                                    - experiences[current_exp_id].x_m),
                                   2)
                       + (double)pow((experiences[waypoint_exp_id].y_m
                                      - experiences[current_exp_id].y_m),
                                     2)));
  }
  ...
  double ExperienceMap::get_subgoal_rad() const {
    if (waypoint_exp_id == -1)
      ...
      double curr_goal_rad = atan2((double)(experiences[waypoint_exp_id].y_m
                                            - experiences[current_exp_id].y_m),
                                   (double)(experiences[waypoint_exp_id].x_m
                                            - experiences[current_exp_id].x_m));
      return (get_signed_delta_rad(experiences[current_exp_id].th_rad,
                                   curr_goal_rad));
    }
  }
