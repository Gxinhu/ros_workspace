...
namespace ratslam {
  ...
  /**
   * @brief 图片进行平均池化、全局归一化和补丁归一化预处理
   *
   * @param view_rgb 图片数据矩阵 灰度图格式就是 W * H
   * @param greyscale  是否为灰度图
  ...
  void LocalViewMatch::on_image(const unsigned char *view_rgb, bool greyscale,
    ...
    if (IMAGE_VT_X_RANGE_MAX == -1) IMAGE_VT_X_RANGE_MAX = IMAGE_WIDTH;
    if (IMAGE_VT_Y_RANGE_MAX == -1) IMAGE_VT_Y_RANGE_MAX = IMAGE_HEIGHT;
