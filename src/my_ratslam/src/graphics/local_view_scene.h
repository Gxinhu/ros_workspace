...
namespace ratslam {
  ...
  public:
    LocalViewScene(ptree &settings, LocalViewMatch *in_vt) {
      ...
      get_setting_from_ptree(vt_window_height, settings, "vt_window_height",
                             480);

      update_ptr(in_vt);

      // the camera image is in the top half and the two template windows in the
      // bottom half
      //  vt_window_height = vtm->IMAGE_HEIGHT *
      //  ((double)vt_window_width/vtm->IMAGE_WIDTH) * 2;
    ...
    void draw_all() {
      ...
      draw_image(vtm->view_rgb, vtm->greyscale, -1.0f, 1.0f, vtm->IMAGE_WIDTH,
                 vtm->IMAGE_HEIGHT, (double)vt_window_width / vtm->IMAGE_WIDTH,
                 -(double)vt_window_width / vtm->IMAGE_WIDTH);

      draw_image((const double *)&vtm->templates[vtm->current_vt].data[0], true,
                 -1, 0.0, vtm->TEMPLATE_X_SIZE, vtm->TEMPLATE_Y_SIZE,
  ...
  private:
    void draw_image(const double *image, bool greyscale, float x, float y,
                    int width, int height, double scale_x, double scale_y) {
      unsigned char *texture_ptr_start
          = (unsigned char *)malloc(width * height * (greyscale ? 1 : 3));

      const double *image_ptr = image;
      const double *image_end
          = image_ptr + width * height * (greyscale ? 1 : 3);
      unsigned char *texture_ptr = texture_ptr_start;
      for (; image_ptr < image_end;) {
        *(texture_ptr++) = (unsigned char)(*(image_ptr++) * 255.0);
      }

      draw_image(texture_ptr_start, greyscale, x, y, width, height, scale_x,
                 scale_y);
      free(texture_ptr_start);
    }

    void draw_image(const unsigned char *image, bool greyscale, float x,
                    float y, int width, int height, double scale_x,
                    double scale_y) {
      glRasterPos2f(x, y);
