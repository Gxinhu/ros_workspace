...
namespace ratslam {
  ...
  public:
    ...
    void draw_all() {
      ...
      draw_image((const double *)&vtm->current_view[0], true, -1.0, -0.5, vtm->TEMPLATE_X_SIZE,
  ...
  private:
    void draw_image(const double *image, bool greyscale, float x, float y, int width, int height,
    ...
    void draw_image(const unsigned char *image, bool greyscale, float x, float y, int width,
                    int height, double scale_x, double scale_y) {
      glRasterPos2f(x, y);
