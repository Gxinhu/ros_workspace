    ...
    PosecellScene(ptree &settings, PosecellNetwork *in_pc)
      ...
      for (int i = 0; i < NUM_PARTICLES; i++) {
        particles[i] = pose_cells_scene->addBillboardSceneNode(
            pose_cells_scene->getRootSceneNode(), irr::core::dimension2d<irr::f32>(2, 2));
        ...
        particles[i]->setMaterialTexture(0, pose_cells_scene->getVideoDriver()->getTexture(
      ...
      // bottom
      pose_cell_boundary->addPoint(irr::core::vector3df(-posecells->PC_DIM_XY / 2.0f,
                                                        -posecells->PC_DIM_TH / 2.0f,
                                                        -posecells->PC_DIM_XY / 2.0f));
      pose_cell_boundary->addPoint(irr::core::vector3df(
          posecells->PC_DIM_XY / 2.0f, -posecells->PC_DIM_TH / 2.0f, -posecells->PC_DIM_XY / 2.0f));
      pose_cell_boundary->addPoint(irr::core::vector3df(
          posecells->PC_DIM_XY / 2.0f, -posecells->PC_DIM_TH / 2.0f, -posecells->PC_DIM_XY / 2.0f));
      pose_cell_boundary->addPoint(irr::core::vector3df(
          posecells->PC_DIM_XY / 2.0f, -posecells->PC_DIM_TH / 2.0f, posecells->PC_DIM_XY / 2.0f));
      ...
      irr::scene::ICameraSceneNode *pose_cell_camera = pose_cells_scene->addCameraSceneNode(
          NULL, irr::core::vector3df(10, 40, -40), irr::core::vector3df(0, 0, 0));
      irr::core::matrix4 proj;
      proj.buildProjectionMatrixOrthoLH((irr::f32)posecells->PC_DIM_XY * 2 + 1, (irr::f32)50,
            ...
            if (pose_cells[next] > 0.002) {
              if (next_particle < NUM_PARTICLES) {
                particles[next_particle]->setPosition(
                    irr::core::vector3df((irr::f32)(i - posecells->PC_DIM_XY / 2),
