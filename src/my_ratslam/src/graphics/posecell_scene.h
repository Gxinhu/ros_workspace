...
namespace ratslam {
  ...
  public:
    PosecellScene(ptree &settings, PosecellNetwork *in_pc)
        : pose_cells_scene(NULL),
          particles(NULL),
          position_line(NULL),
          pose_cell_history(NULL),
          posecells(in_pc) {
      window_width = 400;
      window_height = 400;
      ...
      device = irr::createDevice(
          ...
          irr::core::dimension2d<irr::u32>(window_width, window_height), 32,
          false, false, false);
      ...
      for (int i = 0; i < NUM_PARTICLES; i++) {
        particles[i] = pose_cells_scene->addBillboardSceneNode(
        ...
        particles[i]->getMaterial(0).EmissiveColor
            = irr::video::SColor(255, 255, 255, 255);
        particles[i]->setMaterialType(irr::video::EMT_TRANSPARENT_ADD_COLOR);
        particles[i]->setMaterialTexture(
            0, pose_cells_scene->getVideoDriver()->getTexture(
                   (media_path + "/particle.bmp").c_str()));
      }

      position_line = pose_cells_scene->addMeshSceneNode(
          pose_cells_scene->getGeometryCreator()->createCylinderMesh(
              0.5, 1.0, 5, irr::video::SColor(255, 255, 0, 0)),
          pose_cells_scene->getRootSceneNode());
      position_line->getMaterial(0).Lighting = false;
      position_line->getMaterial(0).EmissiveColor
          = irr::video::SColor(255, 255, 0, 0);

      pose_cell_history = new PathNode(pose_cells_scene->getRootSceneNode());
      pose_cell_history->getMaterial(0).EmissiveColor
          = irr::video::SColor(255, 0, 255, 0);
      pose_cell_history->setPrimitiveType(irr::scene::EPT_POINTS);

      PathNode *pose_cell_boundary
          = new PathNode(pose_cells_scene->getRootSceneNode());
      pose_cell_boundary->setPrimitiveType(irr::scene::EPT_LINES);
      pose_cell_boundary->getMaterial(0).EmissiveColor
          = irr::video::SColor(255, 255, 255, 255);
      pose_cell_boundary->getMaterial(0).Thickness = 2.0f;
      // bottom
      pose_cell_boundary->addPoint(irr::core::vector3df(
          -posecells->PC_DIM_XY / 2.0f, -posecells->PC_DIM_TH / 2.0f,
          -posecells->PC_DIM_XY / 2.0f));
      pose_cell_boundary->addPoint(irr::core::vector3df(
          posecells->PC_DIM_XY / 2.0f, -posecells->PC_DIM_TH / 2.0f,
          -posecells->PC_DIM_XY / 2.0f));
      pose_cell_boundary->addPoint(irr::core::vector3df(
      ...
      irr::scene::ICameraSceneNode *pose_cell_camera
          = pose_cells_scene->addCameraSceneNode(
              NULL, irr::core::vector3df(10, 40, -40),
              irr::core::vector3df(0, 0, 0));
      irr::core::matrix4 proj;
      proj.buildProjectionMatrixOrthoLH((irr::f32)posecells->PC_DIM_XY * 2 + 1,
                                        (irr::f32)50, (irr::f32)0.01,
                                        (irr::f32)100);
          ...
          for (int i = 0; i < PC_DIM_XY; i++) {
            if (pose_cells[next] > 0.002) {
              if (next_particle < NUM_PARTICLES) {
                particles[next_particle]->setPosition(irr::core::vector3df(
                    (irr::f32)(i - posecells->PC_DIM_XY / 2),
    ...
    void update_position(double x, double y, double th, int PC_DIM_XY,
                         int PC_DIM_TH) {
      position_line->setPosition(
          irr::core::vector3df((irr::f32)(x - (double)PC_DIM_XY / 2),
                               (irr::f32)(th - (double)PC_DIM_TH / 2),
                               (irr::f32)(y - (double)PC_DIM_XY / 2)));

      position_line->setScale(
          irr::core::vector3df((irr::f32)0.5, (irr::f32)-th, (irr::f32)0.5));

      // TODO free points after eg 1000 points
      pose_cell_history->addPoint(
          irr::core::vector3df((irr::f32)(x - (double)PC_DIM_XY / 2),
    ...
    void update_scene() {
      update_pose_cells(posecells->posecells_memory, posecells->PC_DIM_XY,
                        posecells->PC_DIM_TH);
      update_position(posecells->best_x, posecells->best_y, posecells->best_th,
                      posecells->PC_DIM_XY, posecells->PC_DIM_TH);
    }

    void clear_history() { pose_cell_history->clearPoints(); }
