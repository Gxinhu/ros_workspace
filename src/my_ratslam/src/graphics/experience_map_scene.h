  ...
  public:
    ExperienceMapScene(ptree &settings, ExperienceMap *in_map)
      ...
      irr::scene::ICameraSceneNode *camera = scene->addCameraSceneNode(
          ...
          irr::core::vector3df((irr::f32)(0.5 * width),
                               (irr::f32)(0.5 * height), -10),
          irr::core::vector3df((irr::f32)(0.5 * width),
                               (irr::f32)(0.5 * height), 0));
      ...
      exp_map_path->getMaterial(0).EmissiveColor
      ...
      exp_map_goal_path->getMaterial(0).EmissiveColor
          = irr::video::SColor(255, 255, 0, 0);
      exp_map_goal_path->getMaterial(0).Thickness = 2.0f;

      // todo: use relative path here and if can't find the image then draw a
      // simple robot shape box
      get_setting_from_ptree(media_path, settings, "media_path",
                             (std::string) "");
      get_setting_from_ptree(image_file, settings, "image_file",
                             (std::string) "");

      // add the irat texture
      irr::video::ITexture *irat_texture
          = exp_map_scene->getVideoDriver()->getTexture(
              (media_path + "/" + image_file).c_str());
      if (irat_texture == 0) {
        std::cout << "WARNING: Unable to load texture: "
                  << (media_path + "/" + image_file) << std::endl;
      }
        ...
        irat_node = exp_map_scene->addMeshSceneNode(
            exp_map_scene->getGeometryCreator()->createCubeMesh(
                irr::core::vector3df(
                    ...
                    (irr::f32)(irat_texture->getSize().Height / 8.0),
                    (irr::f32)0.1)),
            NULL);
      } else {
        irat_node = exp_map_scene->addMeshSceneNode(
      ...
      irat_node->getMaterial(0).EmissiveColor
          = irr::video::SColor(255, 255, 255, 255);
      if (irat_texture) {
        irat_node->getMaterial(0).setTexture(0, irat_texture);
      }
      irat_node->getMaterial(0).MaterialType
    ...
    void update_links() {
      ...
      for (unsigned int i = 0; i < map->links.size(); i++) {
        x2d1 = (int)(((map->experiences[map->links[i].exp_from_id].x_m - min_x)
                      / (max_x - min_x) * viewport_width));
        y2d1 = (int)(((map->experiences[map->links[i].exp_from_id].y_m - min_y)
                      / (max_y - min_y) * viewport_height));
        x2d2 = (int)(((map->experiences[map->links[i].exp_to_id].x_m - min_x)
                      / (max_x - min_x) * viewport_width));
        y2d2 = (int)(((map->experiences[map->links[i].exp_to_id].y_m - min_y)
                      / (max_y - min_y) * viewport_height));
        exp_map_path->addPoint(
            irr::core::vector3df((irr::f32)x2d1, (irr::f32)y2d1, (irr::f32)0));
        exp_map_path->addPoint(
            irr::core::vector3df((irr::f32)x2d2, (irr::f32)y2d2, (irr::f32)0));
      }
    }

    void update_goal_path(int exp1_id, int exp2_id) {
      int x2d1, y2d1, x2d2, y2d2;
      x2d1 = (int)(((map->experiences[exp1_id].x_m - min_x) / (max_x - min_x)
                    * viewport_width));
      y2d1 = (int)(((map->experiences[exp1_id].y_m - min_y) / (max_y - min_y)
                    * viewport_height));
      x2d2 = (int)(((map->experiences[exp2_id].x_m - min_x) / (max_x - min_x)
                    * viewport_width));
      y2d2 = (int)(((map->experiences[exp2_id].y_m - min_y) / (max_y - min_y)
                    * viewport_height));
      exp_map_goal_path->addPoint(
          irr::core::vector3df((irr::f32)x2d1, (irr::f32)y2d1, (irr::f32)-1.0));
      exp_map_goal_path->addPoint(
          irr::core::vector3df((irr::f32)x2d2, (irr::f32)y2d2, (irr::f32)-1.0));
    }
    ...
    void update_goal_list(const std::deque<int> &goals) {
      ...
      for (i = 0; i < goals.size(); i++) {
        ...
        numberNodes[i] = exp_map_scene->addBillboardTextSceneNode(
            exp_map_scene->getGUIEnvironment()->getBuiltInFont(), buffer, NULL,
            irr::core::dimension2d<irr::f32>(50, 50),
            irr::core::vector3df(
                (irr::f32)((map->experiences[goals[i]].x_m - min_x)
                           / (max_x - min_x) * viewport_width),
                (irr::f32)((map->experiences[goals[i]].y_m - min_y)
                           / (max_y - min_y) * viewport_height),
                (irr::f32)0.0),
            -1, -1);
      }
      if (map->waypoint_exp_id != -1) {
        ...
        numberNodes[i] = exp_map_scene->addBillboardTextSceneNode(
            ...
            irr::core::dimension2d<irr::f32>(50, 50),
            irr::core::vector3df(
                (irr::f32)((map->experiences[map->waypoint_exp_id].x_m - min_x)
                           / (max_x - min_x) * viewport_width),
                (irr::f32)((map->experiences[map->waypoint_exp_id].y_m - min_y)
                           / (max_y - min_y) * viewport_height),
                (irr::f32)0.0),
            -1, -1);
      }
    ...
    void update_current_experience(int exp) {
      float x2d1, y2d1;
      x2d1 = (irr::f32)((map->experiences[exp].x_m - min_x) / (max_x - min_x)
                        * viewport_width);
      y2d1 = (irr::f32)((map->experiences[exp].y_m - min_y) / (max_y - min_y)
                        * viewport_height);
      ...
      if (agent_rad > 0 && agent_rad < M_PI_2) {
        irat_node->setRotation(irr::core::vector3df(
            0, 0,
            (irr::f32)(atan2(facing_ratio * viewport_height, viewport_width)
                       * 180 / M_PI)));
    ...
    void update_viewport(int x, int y, int width, int height) {
      ...
      irr::scene::ICameraSceneNode *exp_map_camera
          = exp_map_scene->addCameraSceneNode(
              NULL,
              irr::core::vector3df((irr::f32)(0.5 * viewport_width),
                                   (irr::f32)(0.5 * viewport_height),
                                   (irr::f32)-40),
              irr::core::vector3df((irr::f32)(0.5 * viewport_width),
                                   (irr::f32)(0.5 * viewport_height),
                                   (irr::f32)0));
      irr::core::matrix4 proj;
      proj.buildProjectionMatrixOrthoLH((irr::f32)(1.0 * viewport_width),
                                        (irr::f32)(1.0 * viewport_height),
                                        (irr::f32)0.01, (irr::f32)100);
    ...
    void set_viewport() {
      exp_map_scene->getVideoDriver()->setViewPort(
          irr::core::rect<irr::s32>((irr::s32)viewport_x, (irr::s32)viewport_y,
    ...
    void update_scene() {
      ...
      if (map->goal_list.size() && map->waypoint_exp_id != -1) {
        ...
        while (trace_exp_id != map->goal_path_final_exp_id) {
          update_goal_path(trace_exp_id,
                           map->experiences[trace_exp_id].goal_to_current);
    ...
    void screen_to_world(int x, int y, double &x_m, double &y_m) {
      ...
      y_m = (double)max_y
            - (y - viewport_y) / viewport_height * (max_y - min_y);
    }

    void update_ptr(ExperienceMap *in_map) { map = in_map; }
