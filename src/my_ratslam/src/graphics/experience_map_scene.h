  ...
  public:
    ExperienceMapScene(ptree &settings, ExperienceMap *in_map)
      ...
      device = irr::createDevice(irr::video::EDT_OPENGL,
                                 irr::core::dimension2d<irr::u32>(width, height), 32, false, false,
                                 false);
      ...
      proj.buildProjectionMatrixOrthoLH((irr::f32)width, (irr::f32)height, (irr::f32)0.01,
                                        (irr::f32)100);
      ...
      if (irat_texture == 0) {
        std::cout << "WARNING: Unable to load texture: " << (media_path + "/" + image_file)
                  << std::endl;
      }
      ...
      if (irat_texture) {
        irat_node = exp_map_scene->addMeshSceneNode(
            exp_map_scene->getGeometryCreator()->createCubeMesh(irr::core::vector3df(
    ...
    void update_links() {
      ...
      for (unsigned int i = 0; i < map->links.size(); i++) {
        x2d1 = (int)(((map->experiences[map->links[i].exp_from_id].x_m - min_x) / (max_x - min_x)
                      * viewport_width));
        y2d1 = (int)(((map->experiences[map->links[i].exp_from_id].y_m - min_y) / (max_y - min_y)
                      * viewport_height));
        x2d2 = (int)(((map->experiences[map->links[i].exp_to_id].x_m - min_x) / (max_x - min_x)
                      * viewport_width));
        y2d2 = (int)(((map->experiences[map->links[i].exp_to_id].y_m - min_y) / (max_y - min_y)
                      * viewport_height));
    ...
    void update_goal_list(const std::deque<int> &goals) {
      ...
      for (i = 0; i < goals.size(); i++) {
        ...
        numberNodes[i] = exp_map_scene->addBillboardTextSceneNode(
            ...
            irr::core::vector3df((irr::f32)((map->experiences[goals[i]].x_m - min_x)
      ...
      if (map->waypoint_exp_id != -1) {
        ...
        numberNodes[i] = exp_map_scene->addBillboardTextSceneNode(
            exp_map_scene->getGUIEnvironment()->getBuiltInFont(), buffer, NULL,
            irr::core::dimension2d<irr::f32>(50, 50),
            irr::core::vector3df((irr::f32)((map->experiences[map->waypoint_exp_id].x_m - min_x)
                                            / (max_x - min_x) * viewport_width),
    ...
    void update_current_experience(int exp) {
      ...
      if (agent_rad > 0 && agent_rad < M_PI_2) {
        irat_node->setRotation(irr::core::vector3df(
            0, 0, (irr::f32)(atan2(facing_ratio * viewport_height, viewport_width) * 180 / M_PI)));
      } else if (agent_rad > M_PI_2 && agent_rad < M_PI) {
        irat_node->setRotation(irr::core::vector3df(
            0, 0,
            (irr::f32)(atan2(-facing_ratio * viewport_height, -viewport_width) * 180 / M_PI)));
    ...
    void update_viewport(int x, int y, int width, int height) {
      ...
      irr::scene::ICameraSceneNode *exp_map_camera = exp_map_scene->addCameraSceneNode(
          NULL,
          irr::core::vector3df((irr::f32)(0.5 * viewport_width), (irr::f32)(0.5 * viewport_height),
                               (irr::f32)-40),
          irr::core::vector3df((irr::f32)(0.5 * viewport_width), (irr::f32)(0.5 * viewport_height),
                               (irr::f32)0));
      irr::core::matrix4 proj;
      proj.buildProjectionMatrixOrthoLH((irr::f32)(1.0 * viewport_width),
                                        (irr::f32)(1.0 * viewport_height), (irr::f32)0.01,
                                        (irr::f32)100);
      exp_map_camera->setProjectionMatrix(proj, true);
    }

    void set_viewport() {
      exp_map_scene->getVideoDriver()->setViewPort(irr::core::rect<irr::s32>(
          (irr::s32)viewport_x, (irr::s32)viewport_y, (irr::s32)(viewport_x + viewport_width),
