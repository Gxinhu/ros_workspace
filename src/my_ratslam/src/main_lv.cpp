...
void image_callback(sensor_msgs::ImageConstPtr image, ros::Publisher *pub_vt) {
  ...
  static ratslam_ros::ViewTemplate
      vt_output;  //创建一个ViewTemplate类型的消息vt_output
   ...
   * 5、然后与存储的template对比是否小于先前定义的阈值?返回匹配到的模板id或新建立的模板id*/
  lv->on_image(&image->data[0], (image->encoding != "bgr8"), image->width,
               image->height);
...
/*---------------------------------------------------------------------------------------------------------
函数功能：订阅topic_root + "/camera/image"，接受topic_root +
"/camera/image/compressed"消息， 然后在话题topic_root +
"/LocalView/Template"发布ratslam_ros::ViewTemplate类型的消息
...
int main(int argc, char *argv[]) {
  ...
  read_ini(argv[1],
           settings);  //读入传入的文件名对应的文件，并保存到setting ptree中
  ...
  {
    ros::init(argc, argv,
              "RatSLAMViewTemplate");  //初始化一个“RatSLAMViewTemplate”节点
  }
  ros::NodeHandle node;  //新建一个NodeHandle节点，调用节点函数

  //使用上一步的NodeHandl对象node在Master端注册一个Publisher,
  // 其中发布消息的类型为ratslam_ros::ViewTemplate，topic名为topic_root
  //+ "/LocalView/Template"
  ros::Publisher pub_vt = node.advertise<ratslam_ros::ViewTemplate>(
      topic_root + "/LocalView/Template", 0);

  image_transport::ImageTransport it(
      node);  //定义一个专用于图像的NodeHandle的对象
  ...
  image_transport::Subscriber sub
      = it.subscribe(topic_root + "/camera/image", 0,
