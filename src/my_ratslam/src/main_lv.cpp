...
int main(int argc, char *argv[]) {
  ...
  //使用上一步的NodeHandl对象node在Master端注册一个Publisher,其中发布消息的类型为ratslam_ros::ViewTemplate，topic名为topic_root
  //+ "/LocalView/Template"
  ros::Publisher pub_vt
      = node.advertise<ratslam_ros::ViewTemplate>(topic_root + "/LocalView/Template", 0);
  ...
  image_transport::Subscriber sub
      = it.subscribe(topic_root + "/camera/image", 0, boost::bind(image_callback, _1, &pub_vt));

#ifdef HAVE_IRRLICHT
  boost::property_tree::ptree draw_settings;
  get_setting_child(draw_settings, settings, "draw", true);
  get_setting_from_ptree(use_graphics, draw_settings, "enable", true);
  if (use_graphics) lvs = new ratslam::LocalViewScene(draw_settings, lv);
#endif
