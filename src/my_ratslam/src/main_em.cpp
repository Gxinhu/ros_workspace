...
void odo_callback(nav_msgs::OdometryConstPtr odo, ratslam::ExperienceMap *em) {
  ...
  if (em->get_current_goal_id() >= 0) {
    // (prev_goal_update.toSec() == 0 || (odo->header.stamp -
    // prev_goal_update).toSec() > 5)
...
void action_callback(ratslam_ros::TopologicalActionConstPtr action,
  ...
  ROS_DEBUG_STREAM("EM:action_callback{"
                   << ros::Time::now() << "} action=" << action->action
  ...
  pose_output.pose.orientation.z
      = sin(em->get_experience(em->get_current_id())->th_rad / 2.0);
  pose_output.pose.orientation.w
      = cos(em->get_experience(em->get_current_id())->th_rad / 2.0);
  ...
  if (action->header.stamp - prev_pub_time > ros::Duration(30.0)) {
    ...
    for (int i = 0; i < em->get_num_links(); i++) {
      ...
      em_map.edge[i].transform.translation.x
          = em->get_link(i)->d * cos(em->get_link(i)->heading_rad);
